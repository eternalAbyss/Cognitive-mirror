...
export default function CognitiveMirror() {
  ...
  useEffect(() => {
    ...
    try { localStorage.setItem("cm-theme", dark ? "dark" : "light"); } catch { /* ignore */ }
  ...
  return (
    <div style={{ width: "100vw", height: "100vh", background: BG, overflow: "hidden", position: "relative", fontFamily: SANS }}>
      ...
      <div id="cm-hud" style={{ position: "absolute", inset: 0, zIndex: 10, perspective: "2000px", transition: "opacity .8s ease,filter .8s ease" }}>
        ...
        <div style={{ position: "absolute", top: 30, left: "50%", transform: "translateX(-50%)", textAlign: "center", pointerEvents: "none" }}>
          <div style={{ fontFamily: SANS, fontWeight: 500, fontSize: 13, letterSpacing: 7, color: ink(.72), textTransform: "uppercase", textShadow: `0 0 12px ${BG},0 0 12px ${BG},0 0 6px ${BG}` }}>The Cognitive Mirror</div>
          <div style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 4, color: ink(.34), textTransform: "uppercase", marginTop: 6, textShadow: `0 0 8px ${BG},0 0 8px ${BG}` }}>Second Brain · {view.nodeReadout}</div>
        </div>

        {/* Clock */}
        <div style={{ position: "absolute", top: 28, left: 28, width: 268, background: surface(.66), backdropFilter: "blur(14px) saturate(1.05)", WebkitBackdropFilter: "blur(14px) saturate(1.05)", border: `1px solid ${ink(.07)}`, borderRadius: 14, padding: "20px 22px", boxShadow: `0 10px 34px rgba(0,0,0,.05),inset 0 1px 0 ${surface(.7)}` }}>
          <div style={{ fontSize: 9, letterSpacing: 3.5, color: ink(.38), textTransform: "uppercase", marginBottom: 5 }}>Good {clock.greeting}</div>
          <div style={{ fontFamily: MONO, fontSize: 48, fontWeight: 700, color: INK, lineHeight: 1, letterSpacing: -2 }}>{clock.time}</div>
          <div style={{ fontFamily: MONO, fontSize: 10.5, color: ink(.42), marginTop: 9, letterSpacing: 0.3 }}>{clock.date}</div>
          <div style={{ marginTop: 11, paddingTop: 10, borderTop: `1px solid ${ink(.08)}`, fontSize: 8, letterSpacing: 3.5, color: ink(.3), textTransform: "uppercase" }}>LOCAL · BENGALURU</div>
        </div>

        {/* Operation log (live maintenance activity) */}
        <div style={{ position: "absolute", top: 192, left: 28, width: 268, maxHeight: 230, overflow: "hidden", background: surface(.6), backdropFilter: "blur(13px) saturate(1.05)", WebkitBackdropFilter: "blur(13px) saturate(1.05)", border: `1px solid ${ink(.07)}`, borderRadius: 14, padding: "14px 16px", boxShadow: "0 8px 30px rgba(0,0,0,.05)" }}>
          <div style={{ fontSize: 7.5, letterSpacing: 3, color: ink(.4), textTransform: "uppercase", marginBottom: 10 }}>Operation Log</div>
          {oplog.length === 0 && <div style={{ fontFamily: MONO, fontSize: 10, color: ink(.3) }}>idle</div>}
          {oplog.slice(0, 7).map((o) => (
            <div key={o.id} style={{ display: "flex", gap: 8, padding: "4px 0", borderBottom: `1px solid ${ink(.05)}` }}>
              <span style={{ fontFamily: MONO, fontSize: 8.5, color: ink(.3), flexShrink: 0 }}>{o.ts.slice(11, 16)}</span>
              <span style={{ fontFamily: MONO, fontSize: 9.5, color: ink(.66), overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{o.reason.replace(/"/g, "")}</span>
            </div>
          ))}
        </div>

        {/* Weather (ambient) */}
        <div style={{ position: "absolute", top: 28, right: 28, width: 204, background: surface(.66), backdropFilter: "blur(14px) saturate(1.05)", WebkitBackdropFilter: "blur(14px) saturate(1.05)", border: `1px solid ${ink(.07)}`, borderRadius: 14, padding: "18px 20px", boxShadow: `0 10px 34px rgba(0,0,0,.05),inset 0 1px 0 ${surface(.7)}` }}>
          <div style={{ fontSize: 7.5, letterSpacing: 3, color: ink(.34), textTransform: "uppercase", marginBottom: 11 }}>Bengaluru</div>
          <div style={{ display: "flex", alignItems: "flex-end", gap: 10, marginBottom: 11 }}>
            <div style={{ fontFamily: MONO, fontSize: 42, fontWeight: 700, color: INK, lineHeight: 1 }}>27°</div>
            <div style={{ paddingBottom: 3 }}>
              <div style={{ fontSize: 12, color: ink(.58), marginBottom: 2 }}>Light rain</div>
              <div style={{ fontFamily: MONO, fontSize: 10, color: ink(.36) }}>≋ ↓ 0.4mm/h</div>
            </div>
          </div>
          <div style={{ display: "flex", gap: 14, paddingTop: 9, borderTop: `1px solid ${ink(.08)}` }}>
            {[["Humid", "78%"], ["Wind", "12km/h"], ["Feels", "25°"]].map(([k, v]) => (
              <div key={k}>
                <div style={{ fontSize: 6.5, letterSpacing: 2, color: ink(.3), textTransform: "uppercase", marginBottom: 3 }}>{k}</div>
                <div style={{ fontFamily: MONO, fontSize: 13, color: ink(.82) }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div id="cm-center" style={{ position: "absolute", top: 150, bottom: 182, left: "50%", width: 548, overflowY: "auto", overflowX: "hidden", transformStyle: "preserve-3d", transition: "transform .25s ease-out, opacity .35s ease", opacity: explore ? 0 : 1, visibility: explore ? "hidden" : "visible" }}>
          <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", justifyContent: "center", gap: 10, padding: "2px 0" }}>
            ...
            {brief.length === 0 && (
              <div style={card}>
                <div style={{ fontSize: 13, color: ink(.55), lineHeight: 1.62 }}>Your graph is empty — add concepts via the MCP server or run <span style={{ fontFamily: MONO, fontSize: 11 }}>pnpm seed</span>, and today's world signal will surface here.</div>
              </div>
            )}
            {brief.map((b, i) => (
            ...
            {loops[0] && (
              <>
                ...
                <div style={{ flexShrink: 0, background: surface(.78), backdropFilter: "blur(13px) saturate(1.05)", WebkitBackdropFilter: "blur(13px) saturate(1.05)", border: `1px solid ${ink(.16)}`, borderRadius: 12, padding: "16px 18px", boxShadow: "0 8px 30px rgba(0,0,0,.07)" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    <div style={{ width: 7, height: 7, background: INK, transform: "rotate(45deg)", flexShrink: 0, animation: "soft-pulse 2.4s ease-in-out infinite" }} />
                    <span style={{ fontSize: 7.5, letterSpacing: 2.5, color: ink(.6), textTransform: "uppercase" }}>Open Loop · resurfacing</span>
                  </div>
                  <div style={{ fontSize: 13, color: ink(.82), lineHeight: 1.62, marginBottom: 14 }}>{loops[0].summary}</div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <button onClick={() => e()?.runDefault()} className="cm-continue" style={loopBtn(ACCENT, ACCENT_FG, ACCENT)}>Continue</button>
                    <button className="cm-park" style={loopBtn("transparent", ink(.5), ink(.18))}>Park</button>
                  </div>
                </div>
              </>
            )}
        ...
        <div style={{ position: "absolute", bottom: 76, left: "50%", transform: "translateX(-50%)", width: 600, display: "flex", flexDirection: "column", alignItems: "center" }}>
          <button onClick={() => { const next = !explore; setExplore(next); if (next) e()?.enterGraph(); else e()?.exitGraph(); }} className="cm-enter" style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 3, textTransform: "uppercase", color: explore ? ACCENT_FG : ink(.62), background: explore ? ACCENT : surface(.7), border: `1px solid ${ink(.16)}`, borderRadius: 20, padding: "7px 18px", marginBottom: explore ? 7 : 13, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, backdropFilter: "blur(10px)", animation: explore ? "none" : "breathe 3.6s ease-in-out infinite" }}>
            <span style={{ fontSize: 10, lineHeight: 1 }}>◈</span> {explore ? "Exit the graph" : "Enter the graph"}
          </button>
          {explore && (
            <div style={{ marginBottom: 12, fontFamily: MONO, fontSize: 7.5, letterSpacing: 2.5, color: ink(.4), textTransform: "uppercase" }}>drag to orbit · scroll to zoom · click a node</div>
          )}
          <form onSubmit={(ev) => { ev.preventDefault(); e()?.submit(); }} style={{ position: "relative", width: "100%" }}>
            <input
              ...
              className="cm-input"
              style={{ width: "100%", background: surface(.82), border: `1px solid ${ink(.14)}`, borderRadius: 28, padding: "15px 54px 15px 24px", fontFamily: SANS, fontSize: 14, color: INK, boxShadow: `0 10px 30px rgba(0,0,0,.06),inset 0 1px 0 ${surface(.8)}`, backdropFilter: "blur(12px)" }}
            />
            <button type="submit" className="cm-submit" style={{ position: "absolute", right: 13, top: "50%", transform: "translateY(-50%)", background: ACCENT, border: "none", borderRadius: "50%", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none"><path d="M2 7h10M8.5 4l3 3-3 3" stroke={ACCENT_FG} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
            </button>
          </form>
          <div style={{ textAlign: "center", marginTop: 8, fontFamily: MONO, fontSize: 7.5, letterSpacing: 3, color: ink(.26), textTransform: "uppercase" }}>{queryHint}</div>
        </div>
        ...
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 56, background: surface(.78), borderTop: `1px solid ${ink(.08)}`, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", display: "flex", alignItems: "center", padding: "0 28px" }}>
          ...
          <div style={{ paddingRight: 20, textAlign: "right" }}>
            <div style={{ fontFamily: MONO, fontSize: 14, color: ink(.5), lineHeight: 1 }}>{spend != null ? `$${spend.toFixed(2)}` : "—"}</div>
            <div style={{ fontSize: 6.5, letterSpacing: 2, color: ink(.28), textTransform: "uppercase" }}>API Today</div>
          </div>
          <Divider />
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 7, height: 7, borderRadius: "50%", background: healthy ? INK : "#C2557A", animation: "soft-pulse 2.4s ease-in-out infinite" }} />
            <span style={{ fontSize: 8, letterSpacing: 2.5, color: ink(.55), textTransform: "uppercase" }}>{healthy ? "Healthy" : "Degraded"}</span>
          </div>
        </div>
      </div>
      ...
      {view.showAnswer && (
        <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", zIndex: 20, width: 760, maxWidth: "92vw", maxHeight: "78vh", display: "flex", flexDirection: "column", background: surface(.92), backdropFilter: "blur(28px) saturate(1.1)", WebkitBackdropFilter: "blur(28px) saturate(1.1)", border: `1px solid ${ink(.1)}`, borderRadius: 16, padding: "26px 30px", boxShadow: "0 30px 80px rgba(0,0,0,.14)", animation: "float-in-center .55s ease-out" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexShrink: 0 }}>
            <div style={{ width: 18, height: 2, background: INK }} />
            <div style={{ fontSize: 7, letterSpacing: 5, color: ink(.5), textTransform: "uppercase" }}>{view.noMatch ? "Not in your graph" : "Retrieved"}</div>
            ...
            {view.sources.length > 0 && (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 20 }}>
                {view.sources.map((src, i) => (
                  <div key={`${src.id}-${i}`} onClick={() => src.id && openNode(src.id)} className="cm-chip" style={{ fontFamily: MONO, fontSize: 9, padding: "5px 13px", background: ink(.03), border: `1px solid ${ink(.16)}`, borderRadius: 20, color: ink(.62), cursor: src.id ? "pointer" : "default", userSelect: "none" }}>{src.label}</div>
                ))}
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 10, marginTop: 20, flexShrink: 0 }}>
            {view.noMatch && (
              <button onClick={() => e()?.researchCurrent()} className="cm-continue" style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 2.5, padding: "9px 22px", background: ACCENT, border: `1px solid ${ACCENT}`, borderRadius: 7, color: ACCENT_FG, cursor: "pointer", textTransform: "uppercase" }}>◈ Research the web</button>
            )}
            <button onClick={() => e()?.closeAnswer()} className="cm-dismiss" style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 2.5, padding: "9px 22px", background: "transparent", border: `1px solid ${ink(.16)}`, borderRadius: 7, color: ink(.5), cursor: "pointer", textTransform: "uppercase" }}>Dismiss</button>
          </div>
        </div>
      )}
      ...
      {showTrace && (
        <div style={{ position: "absolute", top: 200, right: 28, width: 326, bottom: 150, zIndex: 12, display: "flex", flexDirection: "column", background: surface(.82), backdropFilter: "blur(18px) saturate(1.1)", WebkitBackdropFilter: "blur(18px) saturate(1.1)", border: `1px solid ${ink(.1)}`, borderRadius: 14, boxShadow: "0 16px 50px rgba(0,0,0,.1)", overflow: "hidden" }}>
          <div style={{ padding: "14px 16px 12px", borderBottom: `1px solid ${ink(.07)}` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
              <span style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 3, color: ink(.55), textTransform: "uppercase" }}>Reasoning Trace</span>
              <div style={{ display: "flex", gap: 6 }}>
                <button onClick={() => e()?.cycleSpeed()} style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 1, padding: "3px 8px", border: `1px solid ${ink(.16)}`, borderRadius: 6, background: "transparent", color: ink(.6), cursor: "pointer" }}>{view.speedLabel}</button>
                <button onClick={() => e()?.replay()} style={{ fontFamily: MONO, fontSize: 10, lineHeight: 1, padding: "3px 9px", border: `1px solid ${ink(.16)}`, borderRadius: 6, background: "transparent", color: ink(.6), cursor: "pointer" }}>↻</button>
              </div>
            </div>
            <div style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 1.5, color: ink(.36), textTransform: "uppercase", marginTop: 8, minHeight: 10 }}>{view.traverseStage}</div>
          </div>
          <div style={{ flex: 1, overflowY: "auto", padding: "4px 16px 14px" }}>
            {view.traceLines.map((line, i) => (
              <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "8px 0", borderBottom: `1px solid ${ink(.05)}`, animation: "float-in .34s ease-out" }}>
                <span style={{ fontFamily: MONO, fontSize: 10, fontWeight: 700, color: line.accent, flexShrink: 0, lineHeight: 1.5 }}>{line.n}</span>
                <div>
                  <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: 0.4, color: ink(.82), lineHeight: 1.5 }}>{line.label}</div>
                  {line.detail && <div style={{ fontFamily: SANS, fontSize: 11, color: ink(.5), lineHeight: 1.5, marginTop: 3, textWrap: "pretty" as CSSProperties["textWrap"] }}>{line.detail}</div>}
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Active node callout */}
      {view.callout && (
        <div id="cm-callout" style={{ position: "absolute", zIndex: 13, transform: "translate(-50%,-138%)", pointerEvents: "none", background: `rgba(var(--accent),.9)`, border: `1px solid ${`rgba(var(--accent-fg),.14)`}`, borderRadius: 9, padding: "9px 12px", minWidth: 150, maxWidth: 236, boxShadow: "0 12px 30px rgba(0,0,0,.3)", transition: "opacity .25s ease" }}>
          <div style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 1.5, color: ACCENT_FG, textTransform: "uppercase", lineHeight: 1.4 }}>{view.callout.tag} · <span style={{ color: `rgba(var(--accent-fg),.5)` }}>{view.callout.type}</span></div>
          <div style={{ fontFamily: SANS, fontSize: 11, color: `rgba(var(--accent-fg),.74)`, lineHeight: 1.45, marginTop: 5, textWrap: "pretty" as CSSProperties["textWrap"] }}>{view.callout.line}</div>
        </div>
      )}
    </div>
  );
...
function Vital({ value, label, last, dot, onClick }: { value: string; label: string; last?: boolean; dot?: boolean; onClick?: () => void }) {
  return (
    <div onClick={onClick} title={onClick ? "view all" : undefined} style={{ display: "flex", alignItems: "center", gap: 8, paddingRight: last ? 0 : 20, cursor: onClick ? "pointer" : "default" }}>
      ...
      <div>
        <div style={{ fontFamily: MONO, fontSize: 15, fontWeight: 700, color: INK, lineHeight: 1, textDecoration: onClick ? "underline dotted rgba(var(--ink),.25)" : "none", textUnderlineOffset: 3 }}>{value}</div>
        <div style={{ fontSize: 6.5, letterSpacing: 2, color: ink(.36), textTransform: "uppercase" }}>{label}</div>
      </div>
    </div>
  );
}
