...
export async function runMaintenance(graph: GraphClient): Promise<MaintenanceReport> {
  ...
  for (const c of candidates) {
    ...
    log.info({ a: c.aTitle, b: c.bTitle, verdict: adj.verdict, distance: Number(c.distance.toFixed(3)) }, "adjudicated");

    if (adj.verdict === "merge") {
      const ops: GraphOp[] = [];
      if (adj.mergedSummary) {
        ops.push({ kind: "updateNode", id: c.bId, patch: { summary: adj.mergedSummary, ...(adj.mergedTitle ? { title: adj.mergedTitle } : {}) } });
      ...
      const syn = await synthesizeContradiction({ title: c.aTitle, summary: c.aSummary }, { title: c.bTitle, summary: c.bSummary });
      const sid = randomUUID();
      await graph.execute(
        [
          { kind: "createNode", id: sid, node: { type: "Synthesis", title: syn.title, summary: syn.summary } },
          { kind: "setSummaryEmbedding", id: sid, embedding: await embed(syn.summary || syn.title) },
      ...
      await graph.execute(
        [{ kind: "createEdge", from: c.aId, to: c.bId, type: "RELATES_TO", props: { reason: adj.reason } }],
  ...
  for (const p of pairs.slice(0, cfg.MAX_INSIGHTS_PER_RUN)) {
    const ins = await synthesizeInsight({ title: p.aTitle, domain: p.aDomain }, { title: p.bTitle, domain: p.bDomain });
    if (!ins.title) continue; // model judged it shallow
    const iid = randomUUID();
    await graph.execute(
      [
        { kind: "createNode", id: iid, node: { type: "Insight", title: ins.title, summary: ins.summary, domain: `${p.aDomain}×${p.bDomain}` } },
...
async function pruneArchival(graph: GraphClient): Promise<number> {
  ...
  for (const type of ["Conversation", "WorldEvent"]) {
    ...
    if (ops.length) {
      await graph.execute(ops, `archive ${ops.length} stale ${type} (>${cfg.ARCHIVE_INACTIVITY_DAYS}d inactive)`);
      archived += ops.length;
    }
  }
  return archived;
