...
import { childLogger, JOB_TYPE_MAINTENANCE, JOB_TYPE_WORLD_BRIEF, loadConfig } from "@cm/shared";
...
export function startScheduler(queue: JobQueue): () => void {
  ...
  // node-cron 4 widened stop() to `void | Promise<void>`; teardown stays
  // synchronous here, so the shutdown promise is intentionally not awaited.
  return () =>
    tasks.forEach((t) => {
      void t.stop();
    });
